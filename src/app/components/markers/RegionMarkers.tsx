...
function RegionMarkersComponent({
  ...
  const [regionOpacities, setRegionOpacities] = useState<
  ...
  useEffect(() => {
    ...
    const newRegionIds = regions
      .filter(region => !fadeInRegions.has(region.id))
      .map(r => r.id);
  ...
  const handleContextMenu = useCallback(
    (e: L.LeafletMouseEvent, type: 'marker' | 'map', region?: Region) => {
  ...
  const renderRegionMarker = useCallback(
    (
      region: Region,
      opts: {
        onClick: () => void;
      ...
      const centroid = calculatePolygonCenter(region.geom);
      const shouldFadeIn = fadeInRegions.has(region.id);
      const currentOpacity =
        regionOpacities[region.id] ?? (shouldFadeIn ? 0 : 0.8);
      ...
      if (
        ...
        region.labelPosition.direction !== 'Center'
      ) {
        ...
        const offset = calculateLabelOffset(
          region,
          defaultLabelSize.width,
          defaultLabelSize.height
        );
      ...
      if (isPreview && labelIcon) {
        labelIcon.options.className =
          (labelIcon.options.className || '') + ' preview';
      }
        ...
        <>
          {!opts.isZooming && (
            <Polygon
              ...
              positions={region.geom}
              pathOptions={{
                color: region.color,
                weight: region.showBorder ? 2 : 0,
                opacity: currentOpacity,
                fillOpacity:
                  region.showHighlight !== false
                    ? shouldFadeIn
                      ? currentOpacity * 0.25
                      : 0.2
                    : 0,
              ...
              eventHandlers={{
                ...
                click: (e: L.LeafletMouseEvent) => {
                  ...
                  const allContainingRegions = findContainingRegions(
                    clickPoint,
                    regions
                  );
