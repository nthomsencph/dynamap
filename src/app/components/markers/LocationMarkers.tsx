...
import {
  createLocationIcon,
  createLocationLabelDivIcon,
} from '@/app/utils/mapIcons';
...
function LocationMarkersComponent({
  ...
  const handleContextMenu = useCallback(
    (e: L.LeafletMouseEvent, type: 'marker' | 'map', location?: Location) => {
  ...
  const renderLocationMarker = useCallback(
    (
      ...
      const showLabel =
        location.showLabel !== false && (location.label || location.name);

      // Calculate label position based on alignment
      let labelPosition = location.geom;
      if (
        showLabel &&
        location.labelPosition &&
        location.labelPosition.direction !== 'Center'
      ) {
        ...
        labelPosition = applyLabelOffset(location.geom, offset);
      }

      // Create label icon with preview styling if needed
      const labelIcon = showLabel
        ? createLocationLabelDivIcon(location, opts.currentZoom)
        : null;
