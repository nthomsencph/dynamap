      ...
      >
        ...
        <div className="dialog-tabs">
          {(['Content', 'Styling', 'Fields'] as DialogTab[]).map(tab => {
            const isDisabled =
              tab !== 'Content' && validateRequiredFields() !== null;
            return (
              <button
                key={tab}
                type="button"
                ...
                title={
                  isDisabled
                    ? 'Please fill out Name and Type fields first to access this tab'
                    : ''
                }
              >
        ...
        >
          {activeTab === 'Content' && (
            <div className="dialog-tab-content">
              ...
              <div className="dialog-field">
                <Tooltip
                  text="Categorize this element by type (e.g., 'Castle', 'Village'). You can select from common types or type your own."
                  position="right"
                />
              ...
              <div className="dialog-field">
                <DescriptionEditor
                  ...
                  elements={
                    [...locations, ...regions] as Array<Location | Region>
                  }
                  {...richTextEditorProps}
                />
              </div>
              ...
              <div className="dialog-field">
                <Tooltip
                  text="Optional: Add an image that will be displayed when this element is selected"
                  position="right"
                />
                <input
                  type="text"
            ...
            <div className="dialog-tab-content">
              ...
              <div className="dialog-row">
                <label className="dialog-row-label" htmlFor="showLabel">
                  ...
                  <Tooltip
                    text="Toggle whether to display the element's name as a label"
                    position="right"
                  />
                </label>
                <Toggle
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    setForm(prev => ({ ...prev, showLabel: e.target.checked }))
                  }
                  aria-label="Show label"
                />
              </div>
              <div
                className={`label-settings-container ${form.showLabel === true ? 'expanded' : 'collapsed'}`}
              >
                <div className="dialog-row">
                  <label className="dialog-row-label" htmlFor="labelDirection">
                    <span>Label direction</span>
                    <Tooltip
                      text="Choose where to position the label relative to the icon"
                      position="right"
                    />
                  </label>
                        ...
                        onClick={() =>
                          setForm(prev => ({
                            ...
                            labelPosition: {
                              direction: 'Left top',
                              offset:
                                (prev.labelPosition as any)?.offset || 10.0,
                            },
                          }))
                        }
                        title="Left top"
                ...
                <div className="dialog-row">
                  <label className="dialog-row-label" htmlFor="labelOffset">
                    ...
                    <Tooltip
                      text="Adjust the distance between the icon and its label"
                      position="right"
                    />
                  </label>
                  <div className="dialog-slider-wrapper">
                    <input
                      ...
                      onChange={e =>
                        setForm(prev => ({
                          ...
                          labelPosition: {
                            direction:
                              (prev.labelPosition as any)?.direction ||
                              'Center',
                    ...
                    <span className="dialog-slider-value">
                      {form.labelPosition?.offset || 10.0}
                    </span>
                  </div>
                </div>
              </div>
                ...
                <div className="dialog-slider-wrapper">
                  <div className="dual-range-slider">
                    <input
                      ...
                      onChange={e =>
                        handleProminenceChange('lower', Number(e.target.value))
                      }
                    ...
                    <input
                      ...
                      onChange={e =>
                        handleProminenceChange('upper', Number(e.target.value))
                      }
                      className="range-slider range-upper"
                    />
                  </div>
                  <span className="dialog-slider-value">
                    {(form.prominence as any)?.lower ?? 0}-
                    {(form.prominence as any)?.upper ?? 5}
                  </span>
                </div>
              </div>
              <div className="dialog-row">
              ...
              <div className="dialog-row">
                ...
                >
                  {form.icon && ELEMENT_ICONS[form.icon as ElementIcon] ? (
                    React.createElement(
                      ELEMENT_ICONS[form.icon as ElementIcon].icon,
                      {
                        size: 32,
              ...
              {showIconGallery && (
                <div
                  className="icon-gallery-overlay"
                  onClick={() => setShowIconGallery(false)}
                >
                  <div
                    className="icon-gallery-popup"
                    onClick={e => e.stopPropagation()}
                  >
                    ...
                    <div className={`icon-picker-grid ${iconGalleryBg}`}>
                      {Object.entries(ELEMENT_ICONS).map(
                        ([key, { icon: Icon, label }]) => (
                          <button
                            key={key}
                            type="button"
                    ...
                    {field.type === 'range' ? (
                      <div className="dialog-slider-wrapper">
                        <input
                          ...
                          onChange={e =>
                            setForm(prev =>
                              field.onChange(prev, Number(e.target.value))
                            )
                          }
                        />
                        <span className="dialog-slider-value">
                      ...
                      <Toggle
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          setForm(prev =>
                            field.onChange(prev, e.target.checked)
                          )
                        }
            ...
            <div className="dialog-tab-content">
              <div className="fields-section">
                <div className="fields-list">
                  {(
                    Object.entries(form.fields || {}) as [string, string][]
                  ).map(([key, value]) => (
