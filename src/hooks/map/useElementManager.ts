...
import {
  ...
  useUpdateRegion,
  useDeleteRegion,
} from '@/hooks/queries/useMapElements';
...
export function useElementManager() {
  ...
  const mutationHandlers = useMemo(
    () => ({
      updateLocation: (location: Location) =>
        updateLocationMutation.mutateAsync(location as any),
      addLocation: (location: Location) =>
        createLocationMutation.mutateAsync(location as any),
      updateRegion: (region: Region) =>
        updateRegionMutation.mutateAsync(region as any),
      addRegion: (region: Region) =>
        createRegionMutation.mutateAsync(region as any),
    }),
    [
      updateLocationMutation.mutateAsync,
      createLocationMutation.mutateAsync,
      updateRegionMutation.mutateAsync,
      createRegionMutation.mutateAsync,
    ]
  );
  ...
  const handleDeleteElement = useCallback(async () => {
    ...
    deleteConfirm.type,
    deleteLocationMutation.mutateAsync,
    deleteRegionMutation.mutateAsync,
  ]);
